...
attmap <- reactive({
  ...
  mapview(toplo, fill = F, homebutton = F, popup = NULL, legend = F) %>% 
    ...
    addLegend("bottomright", pal = palyes, title = "Station target met", labels = thrsel, opacity = 1, values = yesval) %>% 
    addLegend("bottomright", pal = palno, title = "Station target not met", labels = thrsel, opacity = 1, values = noval)
