...
selplo <- reactive({
  ...
  p2 <- ggplot(toplo, aes(x = Date, y = sd_m)) + 
    ...
    geom_point(size = 0.5, colour = "#0047FE") + 
    labs(
      y = 'Secchi depth (m)', 
      x = NULL, 
  ...
  p3 <-  plot_ly(algplo, x = ~ yrqrt, y= ~ other, type = 'bar', name = 'other', color = cols['other'], text = ~paste0('other, ', yrqrt), hoverinfo = 'text') %>% 
    add_trace(y = ~`Tripos hircus`, name = 'Tripos hircus', color = cols['Tripos hircus'], text = ~paste0('Tripos hircus, ', yrqrt), hoverinfo = 'text') %>% 
    add_trace(y = ~`Pyrodinium bahamense`, name = 'Pyrodinium bahamense', color = cols['Pyrodinium bahamense'], text = ~paste0('Pyrodinium bahamense, ', yrqrt), hoverinfo = 'text') %>% 
    add_trace(y = ~`Pseudo-nitzschia sp.`, name = 'Pseudo-nitzschia sp.', color = cols['Pseudo-nitzschia sp.'], text = ~paste0('Pseudo-nitzschia sp., ', yrqrt), hoverinfo = 'text') %>% 
    add_trace(y = ~`Pseudo-nitzschia pungens`, name = 'Pseudo-nitzschia pungens', color = cols['Pseudo-nitzschia pungens'], text = ~paste0('Pseudo-nitzschia pungens, ', yrqrt), hoverinfo = 'text') %>% 
    add_trace(y = ~`Karenia brevis`, name = 'Karenia brevis', color = cols['Karenia brevis'], text = ~paste0('Karenia brevis, ', yrqrt), hoverinfo = 'text') %>% 
    add_trace(y = ~Cyanobacteria, name = 'Cyanobacteria', color = cols['Cyanobacteria'], text = ~paste0('Cyanobacteria, ', yrqrt), hoverinfo = 'text') %>% 
    add_trace(y = ~Bacillariophyta, name = 'Bacillariophyta', color = cols['Bacillariophyta'], text = ~paste0('Bacillariophyta, ', yrqrt), hoverinfo = 'text') %>% 
...
Each bay segment is assigned to one of the three above categories for each year of data from 1975 to the current year.  The results can be viewed in the *DECISION MATRIX* tab. Hovering the mouse pointer over each cell shows details about the data driving the attainment outcome for a given year: 
