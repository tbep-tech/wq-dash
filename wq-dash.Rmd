...
attmap <- reactive({
  ...
  # attainment data for selected year
  attyr <- avedat %>% 
    filter(yr %in% yrsel) %>% 
  ...
  mapview(toplo, fill = F, homebutton = F, popup = NULL, legend = F) %>% 
    ...
    addLegend("bottomright", pal = palyes, title = "Station target<br>met", labels = thrsel, opacity = 1, values = yesval) %>% 
    addLegend("bottomright", pal = palno, title = "Station target<br> not met", labels = thrsel, opacity = 1, values = noval)
