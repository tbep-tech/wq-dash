...
selplo <- reactive({
  ...
  # chl plot
  p1 <- ggplot2::ggplot(toplo, ggplot2::aes(x = Date, y = chla)) + 
    ggplot2::geom_line(ggplot2::aes(colour = 'Chlorophyll-a')) + 
    ggplot2::scale_colour_manual(values = "#427355") + 
    ggplot2::labs(
  ...
  # secchi plot
  p2 <- ggplot2::ggplot() + 
    ggplot2::geom_line(data = toplo, ggplot2::aes(x = Date, y = sd_raw_m, colour = 'Secchi depth')) + 
    ggplot2::labs(
      y = 'Depth (m)', 
      x = NULL, 
      title = paste0('Station ', selsit)
      ) + 
    ggplot2::scale_colour_manual(values = "#0047FE") +
    pthm +
    ggplot2::theme(
      legend.title = ggplot2::element_blank()
    )
  ...
  if(any(!toplo$sd_q))
    p2 <- p2 +
      ggplot2::geom_point(data = toplo[!toplo$sd_q, ], ggplot2::aes(x = Date, y = sd_raw_m, colour = 'Secchi on bottom'), size = 1.5) +
      ggplot2::scale_colour_manual(values = c('#0047FE', 'tomato1'))

  p1 <- plotly::ggplotly(p1, dynamicTicks = T)
  p2 <- plotly::ggplotly(p2, dynamicTicks = T)
  p3 <- plotly::plot_ly(algplo, x = ~ yrqrt, y= ~count, color = ~name, text = ~paste0(name, ', ', yrqrt), hoverinfo = 'text') %>% 
    plotly::add_bars() %>% 
    plotly::layout(
  ...
  out <- plotly::subplot(p3, p1, p2, nrows = 3, shareX = T, titleY = T, which_layout = 1) %>% 
